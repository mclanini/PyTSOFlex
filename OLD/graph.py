...
class PlotFlexibilidade:
    ...
    def obter_borda_flexibilidade(self):
        """
        Obtém a região de flexibilidade.

        Para cada ângulo, seleciona o primeiro evento que ocorreu,
        ...
        for angulo, grupo in (
            ...
            for _, linha in grupo.iterrows():
                if linha["Violação de Tensão"] == "Sim":
                    candidatos.append(("Violação de Tensão", linha))
                if linha["Violação de Fluxo"] == "Sim":
                    candidatos.append(("Violação de Fluxo", linha))
                if linha["Violação de Geração"] == "Sim":
                    candidatos.append(("Violação de Geração", linha))
                if linha["Convergiu"] == "Não":
                    candidatos.append(("Divergência", linha))
            if candidatos:
                evento, linha = candidatos[0]
                linha = linha.copy()
                linha["Evento"] = evento
                borda.append(linha)
        if not borda:
    ...
    def plot(self, evento, cor):
        ...
        flex = self.obter_borda_flexibilidade()
        if borda.empty:
            return None
        ...
        if not flex.empty:
            ...
            hover_flex = [
                (
                    ...
                    f"<b>P:</b> {row.PL:.2f}<br>"
                    f"<b>Q:</b> {row.QL:.2f}<br>"
                    f"<b>Evento Limitante:</b> {row.Evento}"
                )
                for _, row in flex.iterrows()
            ]
            hover_flex.append(hover_flex[0])
            fig.add_trace(
                go.Scatter(
                    x=flex_plot["PL"],
                    y=flex_plot["QL"],
